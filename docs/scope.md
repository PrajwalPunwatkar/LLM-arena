...
**Correction, found when the build broke.** The decision above is wrong on its central premise and was rebuilt. `page.tsx` being a Server Component does _not_ mean there is no `protect()` call site: `@arcjet/next` exports `request()`, which reconstructs the incoming request from headers and cookies precisely so a Server Component, server action, or route handler can call `protect()` directly. Worse than merely wrong, the `proxy.ts` wiring was actively broken. Arcjet's analysis engine is a WebAssembly module whose Node build inlines the `.wasm` behind a rollup virtual-module path (`_virtual/_./wasm/...`); Next middleware runs on the edge, and Turbopack could not resolve that path in the edge bundle, so the dev server never compiled — a flood of `Module not found` on every boot. The `/api/chat` layer (feature 6) never hit this because a route handler bundles for Node, where the path resolves. Fix: the same bot + IP-sliding-window check now runs inside `ThreadPage` via `guardThreadPage()` (`features/threads/thread-protection.ts`), before the catalog fetch and the database read, so a denied request still does no work — the whole point of covering this route. `proxy.ts` reverts to a bare `clerkMiddleware()` importing no Arcjet, which takes the engine out of the edge bundle entirely. A denial can no longer be a bare 429/403 `Response` (a Server Component cannot return one), so it renders `ThreadUnavailable` (`features/threads/thread-unavailable.tsx`): the house notice shape in a calm neutral tone, the plain sentence, and a retry link — the same "never a raw reason, always a sentence and a way out" rule the rest of the app holds. This also lands where Arcjet's own skill points: it says plainly not to call `protect()` in Next.js middleware.
...
- `features/threads/thread-protection.ts`: `guardThreadPage()`, Shield (inherited from the shared client) plus bot detection and an IP-keyed sliding window, run from the thread page's Server Component via `@arcjet/next`'s `request()`. Same denial shape as chat — plain sentence returned (not a `Response`), real reason logged server-side, `isErrored()` lets the request through rather than dying. _See the correction above: this began in `proxy.ts` and was moved because the edge middleware bundle could not build with Arcjet in it._
- `features/threads/thread-unavailable.tsx`: the calm neutral notice `ThreadPage` renders in place of the arena when `guardThreadPage()` denies — the house notice shape without the destructive red, a plain sentence, and a "Try again" link back to the same thread.
- `app/(shell)/t/[threadId]/page.tsx`: calls `guardThreadPage()` first, before the catalog fetch and the database read, and returns `ThreadUnavailable` on a denial.
- `proxy.ts`: a bare `clerkMiddleware()` — no Arcjet import, no `/t/*` callback. Still gates no route by sign-in; that split stands.
