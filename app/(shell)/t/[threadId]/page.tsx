...
import { guardThreadPage } from "@/features/threads/thread-protection";
 ...
 *
 * Before any of that, `guardThreadPage()` runs Arcjet's bot and rate-limit
 * backstop for this un-gated public route, so a denied request never reaches
 * the catalog fetch or the database (features/threads/thread-protection.ts).
 */
export default async function ThreadPage({
  ...
  const denial = await guardThreadPage();
  if (denial) return <ThreadUnavailable message={denial} threadId={threadId} />;
